...
/**
 * All portal fetch calls include this header so the API auth middleware
 * knows to prefer the portal session cookie over the agency one.
 * Both cookies share the same `localhost` domain in dev, so without this
 * signal the middleware would always pick the agency cookie first.
 */
const PORTAL_HEADERS = { "X-Portalpro-App": "portal" } as const;
...
async function apiFetchMessages(
  ...
  const res = await fetch(
    ...
    { credentials: CREDS, headers: PORTAL_HEADERS },
  );
...
async function apiSendMessage(
  ...
  const res = await fetch(`${API_URL}/api/v1/projects/${projectId}/messages`, {
    method: "POST",
    headers: { ...PORTAL_HEADERS, "Content-Type": "application/json" },
    credentials: CREDS,
...
async function apiMarkAllRead(projectId: string): Promise<void> {
  await fetch(`${API_URL}/api/v1/projects/${projectId}/messages/mark-read`, {
    method: "PATCH",
    credentials: CREDS,
...
async function apiFetchThread(
  ...
  const res = await fetch(`${API_URL}/api/v1/messages/${messageId}/thread`, {
    credentials: CREDS,
    headers: PORTAL_HEADERS,
  });
...
// real message id changes (key = realId). React treats the old and new items
// as different elements, unmounts the old bubble, and mounts a new one —
// causing a visible flicker. Keeping a stable `_key` that never changes for
// the lifetime of a message prevents that unmount/remount cycle.
...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div
      className="h-7 w-7 rounded-full text-[10px] text-white font-bold flex items-center justify-center flex-shrink-0"
      ...
      <div className={`max-w-[72%] ${isMine ? "items-end" : "items-start"} flex flex-col`}>
        ...
        <div
          className={[
            ...
            isMine ? "text-white rounded-tr-sm" : "bg-neutral-100 text-neutral-800 rounded-tl-sm",
          ].join(" ")}
          style={isMine ? { backgroundColor: "var(--portal-primary, #1B4D6E)" } : {}}
        >
          {message.content}
        </div>
...
function ThreadDrawer({
  ...
  // Real-time replies forwarded by the parent via custom window events
  useEffect(() => {
    const handler = (e: Event) => {
      const incoming = (e as CustomEvent<MessageResponse>).detail;
      setReplies((prev) => {
        if (prev.some((r) => r.id === incoming.id)) return prev;
        // Own reply: replace optimistic temp in-place (stable key → no flicker)
    ...
    apiSendMessage(projectId, text, parentMessage.id)
      .then((msg) => {
        ...
        setReplies((prev) => {
          ...
          // Replace temp in-place (stable key → no flicker)
...
/**
 * Full two-way messaging panel for portal clients.
 * Manages its own Socket.io connection (the portal messages page is a dedicated
 * full page, not a tab, so there's no need for the ProjectSocketListener pattern).
 */
export function PortalMessagingPanel({
  projectId,
  currentUserId,
  currentUserName,
}: PortalMessagingPanelProps) {
  ...
  // Real-time: top-level messages
  useEffect(() => {
    const unsub = on<MessageResponse>("message:new", (incoming) => {
      if (incoming.threadId) {
        // Route thread replies to the drawer and bump the parent counter
        window.dispatchEvent(
          new CustomEvent(`portalpro:msg:reply:${incoming.threadId}`, { detail: incoming }),
        );
        setMessages((prev) =>
          prev.map((m) =>
            m.id === incoming.threadId ? { ...m, replyCount: (m.replyCount ?? 0) + 1 } : m,
          ),
        );
        return;
      }
  ...
  const loadMessages = useCallback(
    async (pg: number, append = false) => {
      ...
      try {
        ...
        // Mark messages as read so the unread dot on the project page clears
        // when the user navigates back after visiting messages.
        if (pg === 1) apiMarkAllRead(projectId).catch(() => {});
      } finally {
        setLoading(false);
        ...
        {hasMore && (
          <div className="flex justify-center py-2 border-b border-neutral-100">
            <button
              ...
              style={{ color: "var(--portal-primary, #1B4D6E)" }}
